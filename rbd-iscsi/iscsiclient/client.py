...
from cinder.volume.drivers.rbd_iscsi.iscsiclient import ssh
...
loc_opts = [
    cfg.StrOpt('tgt_local_conf_file',
               default='/tmp/',
               help='tgt local conf file.'),
    cfg.StrOpt('remote_tgt_conf_file',
               default="/etc/tgt/",
               help='remote tgt conf file')]

CONF.register_opts(loc_opts)


class KSClient(object, threading.Thread):
...
'''
    ...
    def __init__(self, ip, username, password, conf):
        self.ip = ip.split(',')
    ...
    def _get_iscsi_target(self, ip):
        ...
        try:
            (output, _err) = utils.execute('iscsiadm', '-m', 'discovery', '-t',
                                           'sendtargets', '-p', target_portal,
                                           run_as_root=True)
        except processutils.ProcessExecutionError as ex:
            LOG.error(_LE("ISCSI discovery attempt failed for:%s")
                      % target_portal)
            LOG.error(_LE("Error from iscsiadm -m discovery: %s") % ex.stderr)
            return None
    ...
    def _add_lun_to_conf_file(self, pool, name, lun):
        ...
        local_conf_file = \
            self.configuration.safe_get("tgt_local_conf_file") + lun_file
        bs = dict()
        ...
        for channel in self.channels:
            with open(local_conf_file, 'w') as f:
                ...
                remote_conf_file = \
                    self.configuration.safe_get("remote_tgt_conf_file") + \
                    '%s/%s' % (channel['iscsi_target']['target_iqn'],
                               local_conf_file.split("tmp/")[1])
    ...
    def _delete_lun_from_conf_file(self, lun):
        conf_file = 'lun%d.conf' % lun

        for channel in self.channels:
            remote_conf_file = \
                self.configuration.safe_get("remote_tgt_conf_file") + \
                '%s/%s' % (channel['iscsi_target']['target_iqn'], conf_file)

            remote_bak_file = \
                self.configuration.safe_get("remote_tgt_conf_file") + \
                '%s/last_delete.bak' % (channel['iscsi_target']['target_iqn'])
