...
struct option const long_options[] = {
	...
	{"online", required_argument, NULL, 'a'},
	{NULL, 0, NULL, 0},
};

static char *short_options =
		"dhVL:o:m:t:s:c:l:n:v:b:E:f:y:T:I:Q:u:p:H:F:P:B:Y:O:C:a:S:";
...
int main(int argc, char **argv)
{
	...
	if (mode == MODE_DEVICE) {
		...
		case OP_STATS:
			...
			break;
		case OP_UPDATE:
			rc = verify_mode_params(argc, argv, "LmofytlabPC");
			if (rc) {
	...
	if (portalOps)
		concat_printf(&b, "%sportalOps %s,", concat_delim(&b, ","),
			      portalOps);
	if (online)
		concat_printf(&b, "%sonline=%s", concat_delim(&b, ","), online);
